...
export default function App () {
  ...
  const [countSteve, setCountSteve]=useState(0)
  const [countCreeper, setCountCreeper] = useState(0)                                         
  ...
  const onTilePress = (row, col) =>{
      ...
      if(winner === 1){
        alert("Победил Стив!")
        setCountSteve(countSteve + 1)
        initializeGame()
      }else if(winner===-1){
        alert("Победил Крипер!")
        setCountCreeper(countCreeper + 1)
        initializeGame()
      }else if(winner === 0){
        alert("Ничья!")
        initializeGame()
  ...
  return(
   <ImageBackground source={require('./img/back4.jpg')} style={styles.container}>
     ...
     <View style={{flexDirection:"row"}}>
       ...
       <TouchableOpacity onPress={() => {onTilePress(1,0)}}   style={[styles.tile, {borderTopWidth:0}]}>
        {renderIcon(1,0)}
       </TouchableOpacity>
       <TouchableOpacity onPress={() => {onTilePress(2,0)}}   style={[styles.tile, {borderTopWidth:0, borderRightWidth:0}]}>
        {renderIcon(2,0)}
       </TouchableOpacity>
     </View>
    <View style={{flexDirection:"row"}}>
       <TouchableOpacity onPress={() => {onTilePress(0,1)}}   style={[styles.tile, {borderLeftWidth:0}]}>
        {renderIcon(0,1)}
       </TouchableOpacity>
       <TouchableOpacity onPress={() => {onTilePress(1,1)}}   style={styles.tile}>
        {renderIcon(1,1)}
       </TouchableOpacity>
       <TouchableOpacity onPress={() => {onTilePress(2,1)}}   style={[styles.tile, {borderRightWidth:0}]}>
        {renderIcon(2,1)}
       </TouchableOpacity>
     </View>
     <View style={{flexDirection:"row"}}>
       <TouchableOpacity onPress={() => {onTilePress(0,2)}}   style={[styles.tile, {borderLeftWidth:0,borderBottomWidth:0}]}>
        {renderIcon(0,2)}
       </TouchableOpacity>
       <TouchableOpacity onPress={() => {onTilePress(1,2)}}   style={[styles.tile, {borderBottomWidth:0}]}>
        {renderIcon(1,2)}
       </TouchableOpacity>
     ...
     <View style={{flexDirection:"row"}}>
       <Text style={styles.text}>Стив:{countSteve} </Text>
       <Text style={styles.text}>Крипер:{countCreeper}</Text>
    </View>
